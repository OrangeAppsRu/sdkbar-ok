...
static bool jsb_oksdk_login(JSContext *cx, uint32_t argc, jsval *vp) {
    ...
    if (argc == 3) {
        ...
        //float *b = nullptr;
        //float c = a / (*b);
        //CCLOG("We just divided by zero! %f", c);
        // CRASH !!!
    } else {
        JS_ReportError(cx, "Invalid number of arguments");
        return false;
...
static void cpp_loginResult(int callbackId, bool success, std::string result) {
    cocos2d::Director::getInstance()->getScheduler()->performFunctionInCocosThread([callbackId, success, result] {
        CallbackFrame *loginCallback = CallbackFrame::getById(callbackId);
        if (!loginCallback) {
            printLog("requestResult: callbackId not found!");
            return;
        }
        CCLOG("!!!! cpp_loginResult %d, %s", success, result.c_str());
        ...
        jsval suc = BOOLEAN_TO_JSVAL(success);
        valArr.append(suc);
        jsval res = my_std_string_to_jsval(loginCallback->cx, result);
        valArr.append(res);
...
void Java_org_cocos2dx_javascript_OkPlugin_loginResult(JNIEnv* env, jobject thiz, jint callbackId, jboolean success, jstring result) {
    ...
    cpp_loginResult(callbackId, success, resultStr);
...
static void cpp_requestResult(int callbackId, std::string errStr, std::string resultStr) {
    cocos2d::Director::getInstance()->getScheduler()->performFunctionInCocosThread([callbackId, errStr, resultStr] {
        ...
        if (resultStr.size() > 0) {
            ...
            ConversionResult r = myUTF8ToUTF16(resultStr, result_utf16);
            if(r != conversionOK ||
               !JS_ParseJSON(cb->cx, reinterpret_cast<const char16_t*> (result_utf16.c_str()), (uint32_t) result_utf16.size(), &rval)) {
                printLog("JSON Error");
                printLog(resultStr.c_str());
                if(r == sourceExhausted)
                    printLog("source exhausted");
                else if(r == targetExhausted)
                    printLog("target exhausted");
                else if(r == sourceIllegal)
                    printLog("source illegal");
                const UTF8 *inbeg = reinterpret_cast<const UTF8*>(&resultStr[0]);
                const UTF8 *inend = inbeg + resultStr.length();
                int illegal = checkUTF8LegalSymbols(inbeg, inend);
                if(illegal >= 0) {
                    std::stringstream ss;
                    ss << "illegal symbol:" << illegal << " from " << resultStr.length();
                    printLog(ss.str().c_str());
                    printLog(printUTF8Code(inbeg+illegal).c_str());
                } else {
                    printLog("Illegal symbol not found");
                }
                valArr.append(my_std_string_to_jsval(cb->cx, std::string("JSON Error")));
            ...
            valArr.append(my_std_string_to_jsval(cb->cx, errStr));
            valArr.append(JSVAL_NULL);
        };

        valArr.append(JSVAL_NULL);
        valArr.append(my_std_string_to_jsval(cb->cx, resultStr));
        JS::HandleValueArray funcArgs = JS::HandleValueArray::fromMarkedLocation(2, valArr.begin());
        cb->call(funcArgs);
